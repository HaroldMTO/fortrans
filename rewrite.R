...
stripbang = function(texte)
{
	...
	while (regexpr(stringRE,tt) > 0) {
		...
		# fusion lignes continuÈes et suppression des '!'
...
{
	# critËre d'arrÍt de la rÈcursion
...
{
	# blocin et blocout d√©pendent de comm, blocin d√©pend de blocout
	nul = "^$"
	inc = "^ *#"
	tag = "^ *[0-9]{1,5}"
	comm = "^ *!"
...
if ("ficout" %in% names(cargs)) {
	# ficout : par dÈfaut, est un fichier
